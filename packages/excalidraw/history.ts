...
export class History {
  ...
  public undo(elements: SceneElementsMap, appState: AppState) {
    return this.perform(
      ...
      (entry: HistoryDelta, origin?: HistoryDelta) =>
        this.push(this.redoStack, entry, origin),
    );
  }
  ...
  public redo(elements: SceneElementsMap, appState: AppState) {
    return this.perform(
      ...
      (entry: HistoryDelta, origin?: HistoryDelta) =>
        this.push(this.undoStack, entry, origin),
    );
  }
  ...
  private perform(
    ...
    push: (entry: HistoryDelta, origin?: HistoryDelta) => void,
    ...
    try {
      ...
      while (historyDelta) {
        // preserve the original popped entry so we can carry its timestamp
        // to the inverted entry pushed onto the opposite stack
        const originDelta = historyDelta;
        try {
  ...
  /**
   * Like `History.push` but carries the timestamp from the originating
   * (popped) entry to the new inverted entry so the UI timeline preserves
   * "when did this edit happen" across undo/redo cycles.
   */
  private push(
    stack: HistoryDelta[],
    entry: HistoryDelta,
    ...
    const inversedEntry = HistoryDelta.inverse(entry);
    const originTimestamp = origin && this.entryTimestamps.get(origin);
    this.entryTimestamps.set(
      inversedEntry,
