...
const persistentLaserPointerCursorDotSVG = `<circle cx="22" cy="22" r="2.5" fill="#1b1b1f"/>`;
const persistentLaserPointerCursorDotSVG_dark = `<circle cx="22" cy="22" r="2.5" fill="#fff"/>`;
...
const persistentLaserPointerCursorDataURL_lightMode = `data:${
  MIME_TYPES.svg
},${encodeURIComponent(
  `${laserPointerCursorSVG_tag}${laserPointerCursorIconSVG}${persistentLaserPointerCursorDotSVG}</svg>`,
)}`;
const persistentLaserPointerCursorDataURL_darkMode = `data:${
  MIME_TYPES.svg
},${encodeURIComponent(
  `${laserPointerCursorSVG_tag}${laserPointerCursorBackgroundSVG}${laserPointerCursorIconSVG}${persistentLaserPointerCursorDotSVG_dark}</svg>`,
)}`;
...
export const setCursorForShape = (
  interactiveCanvas: HTMLCanvasElement | null,
  appState: Pick<AppState, "activeTool" | "theme" | "laserMode">,
) => {
  if (!interactiveCanvas) {
    return;
  }
  if (appState.activeTool.type === "selection") {
    ...
    const url =
      appState.laserMode === "persistent"
        ? appState.theme === THEME.LIGHT
          ? persistentLaserPointerCursorDataURL_lightMode
          : persistentLaserPointerCursorDataURL_darkMode
        : appState.theme === THEME.LIGHT
