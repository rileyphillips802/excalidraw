...
const laserPointerPersistentDotSVG = `<circle cx="17" cy="7" r="2.2" fill="#e03131" stroke="#1b1b1f" stroke-width="0.35"/>`;
...
const laserPointerPersistentCursorDataURL_lightMode = `data:${
  ...
  `${laserPointerCursorSVG_tag}${laserPointerPersistentDotSVG}${laserPointerCursorIconSVG}</svg>`,
)}`;
...
const laserPointerPersistentCursorDataURL_darkMode = `data:${
  ...
  `${laserPointerCursorSVG_tag}${laserPointerCursorBackgroundSVG}${laserPointerPersistentDotSVG}${laserPointerCursorIconSVG}</svg>`,
)}`;
...
export const setCursorForShape = (
  ...
  if (appState.activeTool.type === "selection") {
    ...
    const isPersistent = appState.laserPointerMode === "persistent";
    const url =
