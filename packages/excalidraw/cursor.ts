...
// Persistent variant: filled-black wand body and a single bold continuous
// trail curve instead of scattered sparkles, to signal "strokes stay".
const laserPointerPersistentCursorIconSVG = `<path stroke="#1b1b1f" fill="#1b1b1f" d="m7.868 11.113 7.773 7.774a2.359 2.359 0 0 0 1.667.691 2.368 2.368 0 0 0 2.357-2.358c0-.625-.248-1.225-.69-1.667L11.201 7.78 9.558 9.469l-1.69 1.643v.001Z"/><path stroke="#1b1b1f" stroke-width="1.75" stroke-linecap="round" fill="none" d="M1.5 2.5 Q 3.5 6, 5 5 T 7.5 10"/>`;
...
const laserPointerPersistentCursorDataURL_lightMode = `data:${
  ...
  `${laserPointerCursorSVG_tag}${laserPointerPersistentCursorIconSVG}</svg>`,
)}`;
const laserPointerPersistentCursorDataURL_darkMode = `data:${
  MIME_TYPES.svg
},${encodeURIComponent(
  `${laserPointerCursorSVG_tag}${laserPointerCursorBackgroundSVG}${laserPointerPersistentCursorIconSVG}</svg>`,
)}`;
...
export const setCursorForShape = (
  interactiveCanvas: HTMLCanvasElement | null,
  appState: Pick<AppState, "activeTool" | "theme" | "laserPersistent">,
) => {
  if (!interactiveCanvas) {
    return;
  }
  if (appState.activeTool.type === "selection") {
    ...
    const isLight = appState.theme === THEME.LIGHT;
    const url = appState.laserPersistent
      ? isLight
